...
- **再生コントロール**: リスタート、文/単語/Break スキップ、速度調整（0.05刻み）、音量調整（0〜200%ブースト対応、Web Audio API GainNode使用）
...
- **単語アノテーション**（スクリプト内の単語をダブルクリック）:
  ...
  - **/ Break**: 意味の区切りをマーク（黄色の `/` 表示）— 再生時に各 `/` で自動停止、Play で次のセグメントへ進行。Play ボタンの ▾ ドロップダウンで Break モードの ON/OFF を切替可能（データは保持）。Break モードでのスコアリングは現在のセグメントのみ評価
...
**プレイヤー操作（上段）**:
- **Play / Pause**: 再生/一時停止。Break マークがある場合、**▾** ドロップダウンで **Play with breaks /**（各 `/` で自動停止）と **Play full**（Break 無視）を切替可能
- **Loop**（リピートアイコン）: ループ再生の ON/OFF。Play / モードでは現在の Break セグメント内でループ、Play モードではチャンク全体をループ。録音中は無効
- **Restart**: チャンク先頭に戻る
- **/Break スキップ**: Break 境界単位で前後に移動（Break 未設定時は無効）
...
**スクリプト**: 「Show Script」でカラオケ字幕を表示（スクリプト内容の直上に配置）。単語が発話に合わせてカラー表示されます。**単語クリック**でその位置にジャンプ。**単語ホバー**でIPA発音記号と日本語訳を確認できます。**ダブルクリック**でアノテーションメニューを開きます。

**速度 & 音量**（スクリプト下部、縦並びスライダー）:
...
1. **Record**（Break モード時は **Record /**）をクリック → 再生が自動開始されます。Break モードでは現在のセグメント位置から再生、通常モードでは先頭から再生。録音中はすべてのプレイヤー操作がロックされます。
2. 音声に合わせてシャドーイング。Break モードでは次の `/` で再生・録音が自動停止します。
3. **Stop & Score** をクリック → 録音がWhisperで文字起こしされ、参照テキストと比較してスコアが算出されます。Break モードでは現在のセグメントのテキストのみで評価されます。
4. スコアを確認:
